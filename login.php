...
if (isset($_POST['login'])) {
    ...
    if (!empty($email) && !empty($pass)) {
        ...
        if ($row && password_verify($pass, $row['upass'])) {
            ...
            if ($row_admin && sha1($pass) === $row_admin['apass']) {
                $_SESSION['aid'] = $row_admin['aid'];
                $_SESSION['auser'] = $row_admin['auser'];
                $_SESSION['utype'] = 'admin';

                header("location:admin/dashboard.php");
            } else {
                $error = "<p class='alert alert-warning'>Email or Password doesn't match!</p>";
            }
        }
    } else {
